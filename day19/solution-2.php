...
//    }
    ...
    if ($time > 0) {
        ...
        foreach ($priorities as $i => $value) {
            if ($i < 3 && $robots[$i] >= max($scheme[0][$i], $scheme[1][$i], $scheme[2][$i], $scheme[3][$i])) {
                continue;
            }
            $wait = enoughMinerals($scheme[$i], $minerals, $robots);
            if ($wait !== false && $wait <= 5 && $time > $wait) {
                $newPath = $path;
                $newPath[$time - $wait] = [$i, $minerals[3] + $robots[3] * $wait, $robots[2], $counter];
                $newPriorities = $priorities;
//                $newPriorities[$i] -= 2;
//                arsort($newPriorities);
                [$max, $value] = makeTurn($scheme, $newPriorities, addMinerals($robots, removeMinerals($scheme[$i], $minerals), $wait), addRobot($i, $robots), $time - $wait, $newPath);
                $result[$max] = $value;
                $limit--;
            }
